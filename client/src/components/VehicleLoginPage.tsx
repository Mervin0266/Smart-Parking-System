...
const VehicleLoginPage = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!normalized) {
      alert("Please enter your Christ University email");
      return;
    }

    localStorage.setItem("loggedInEmail", normalized);
    localStorage.setItem("token", "dummy-vehicle-token");
    localStorage.setItem("role", "vehicle");
      ...
      <div className="relative z-10 max-w-md w-full bg-white/10 border border-white/20 rounded-3xl shadow-2xl backdrop-blur-xl px-8 py-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-white/70 mt-1">
            Sign in to view your parking slots
          </p>
        </div>
...
export default VehicleLoginPage;
